function   AB_draw_cumulative_imageFscor(RANSAC,RANSAC_PLUS,LGSC,Ours,Dataname)
...
set(gca,'Fontsize',15);
RANSAC = sprintf('RANSAC, Average F-score=%.2f%%', mean(RANSAC)*100);
RANSAC_PLUS = sprintf('RANSAC++, Average F-score=%.2f%%', mean(RANSAC_PLUS)*100);
LGSC = sprintf('LGSC, Average F-score=%.2f%%', mean(LGSC)*100);
Ours= sprintf('Ours, Average F-score=%.2f%%', mean(Ours)*100);


II=legend({RANSAC,RANSAC_PLUS,LGSC,Ours,Dataname});
TitleName = strcat(Dataname,'-F-score');
title(TitleName);  % CIAP2xmut  FE2xmut  PAN2xmut  UAV2xmut  
xlabel('Cumulative Distribution','fontsize',20);ylabel('F-score','fontsize',20);
set(II,'color','none');
set(II,'fontsize',14);
